...
def render_population_playground():
    ...
    st.markdown(
        '<div class="page-title">Population Demographics</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="page-body">Analyze and compare ethnic group proportions and sub-group breakdowns across East of England districts.</div>',
        unsafe_allow_html=True,
    )
    ...
    with col_sidebar:
        ...
        _ra_module.render_research_assistant_widget(key_suffix="pop_sidebar")
    ...
    with tab_pop_cancer:
        ...
        if cancer_df.empty:
            ...
            if pop_cancer_df.empty:
                ...
                pop_cancer_df["% Black"] = (
                    pop_cancer_df["All Blacks (Total)"]
                    / pop_cancer_df["Total Population"]
                ) * 100
                pop_cancer_df["% Mixed"] = (
