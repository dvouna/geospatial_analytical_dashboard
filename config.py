...
class Config:
    ...
    STREAMLIT_SERVER_BASE_URL_PATH: str = os.getenv('STREAMLIT_SERVER_BASE_URL_PATH', '/')
    ...
    def validate(cls) -> tuple[bool, list[str]]:
        ...
        try:
            if cls.STREAMLIT_SERVER_PORT < 1 or cls.STREAMLIT_SERVER_PORT > 65535:
                errors.append(f"Invalid STREAMLIT_SERVER_PORT: {cls.STREAMLIT_SERVER_PORT}")
...
def check_environment():
    ...
    if errors:
        if Config.DEBUG:
            st.warning(f"⚠️ Configuration warnings:\n" + "\n".join(f"• {e}" for e in errors))
